...
# Ask the user to enter a word. v
# Store the input word in a variable. v
...
my_variable = input("Please enter a word: ")

letter_index_dict = {}

for index, character in enumerate(my_variable):
    if character in letter_index_dict:  #  Uses if in to check.
        letter_index_dict[character].append(index)
    else:
        letter_index_dict[character] = [index]    # If it doesn't exist create it.
...
wallet = wallet.replace("$", "")        # removes the $
wallet = int(wallet)                    # makes integer
print("Money I have:", wallet)           

basket = [] # Basket is a list
for item_name, item_price in items_purchase.items():
    item_price = item_price.replace("$", "")    # remove $
    item_price = item_price.replace(",", "")    # removes ,
    item_price = int(item_price)                # makes integer
    
    if item_price <= wallet:
        basket.append(item_name) # add to basket
        wallet = wallet - item_price # update wallet

if len(basket) == 0:
    print("Nothing")
else:
    basket.sort() # sort will sort alphabetically
    print(basket)